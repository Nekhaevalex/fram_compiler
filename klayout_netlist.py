...
class Fram_Netlist():
	...
	def __init__(self, Config):
		...
		print(f"id: {pmos25.terminal.id("g") } ")

		test_device = self.top_circuit.create_device(self.pmos25 , "name")
		self.top_circuit.create_pin("new_pin")
		net1 = self.top_circuit.create_net("new_net1")
		net2 = self.top_circuit.create_net("new_net2")
		net3 = self.top_circuit.create_net("new_net3")
		net4 = self.top_circuit.create_net("new_net4")

		test_device.connect_terminal("d",net1)
		test_device.connect_terminal("g",net2)
		test_device.connect_terminal("s",net3)
		test_device.connect_terminal("b",net4)
	...
	def init_device(self, name , terminals = None , params = None ):
		...
		if (terminals != None ):
			...
			for terminal in terminals:
				...
				print(f'Terminal name: {new_terminal.name} id = {new_terminal.id() }')
